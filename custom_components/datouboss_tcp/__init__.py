...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def async_set_output_source_priority(call: ServiceCall) -> ServiceResponse:
        ...
        if mode not in OUTPUT_SOURCE_PRIORITY_MAP:
            raise ServiceValidationError(
                f"Invalid mode '{mode}'. Valid values: {', '.join(OUTPUT_SOURCE_PRIORITY_MAP)}"
            )
        payload = await runtime.coordinator.async_send_write_command(
            f"POP{OUTPUT_SOURCE_PRIORITY_MAP[mode]}"
        )
    ...
    async def async_set_charger_source_priority(call: ServiceCall) -> ServiceResponse:
        runtime = _get_loaded_runtime_data(hass, call.data[ATTR_CONFIG_ENTRY_ID])
        mode = call.data[ATTR_MODE]
        if mode not in CHARGER_SOURCE_PRIORITY_MAP:
            raise ServiceValidationError(
                f"Invalid mode '{mode}'. Valid values: {', '.join(CHARGER_SOURCE_PRIORITY_MAP)}"
            )
        payload = await runtime.coordinator.async_send_write_command(
            f"PCP{CHARGER_SOURCE_PRIORITY_MAP[mode]}"
        )
        return {"payload": payload}
    ...
    async def async_set_ac_input_range(call: ServiceCall) -> ServiceResponse:
        runtime = _get_loaded_runtime_data(hass, call.data[ATTR_CONFIG_ENTRY_ID])
        mode = call.data[ATTR_MODE]
        if mode not in AC_INPUT_RANGE_MAP:
            raise ServiceValidationError(
                f"Invalid mode '{mode}'. Valid values: {', '.join(AC_INPUT_RANGE_MAP)}"
            )
        payload = await runtime.coordinator.async_send_write_command(
            f"PGR{AC_INPUT_RANGE_MAP[mode]}"
        )
        return {"payload": payload}
